...
#         aps = [ps_.mean() for ps_ in area_ps] # 平均精度aps（ap）
#         # ps_curve = [
...
def makeplot(rs, ps, outDir, class_name, iou_type, ax):
    ...
    for i in range(len(areaNames)):
        area_ps = ps[..., i, 0]
        figure_title = iou_type + '-' + class_name + '-' + areaNames[i]
        aps = [ps_.mean() for ps_ in area_ps[1:2]]  # average precision (ap) for 'C50'
        ...
        ax.plot(rs, ps_curve[0], label=f'{class_name} {aps[0]:.3f}')  # plot(recall, precision)
        ...
        ax.set_title('Precision-Recall Curve for ' + figure_title)

    fig = ax.get_figure()
    fig.savefig(outDir + '/PR_curve.png', dpi=250)
    plt.close(fig)
...
def analyze_results(res_file,
    ...
    for res_type in res_types:
        ...
        with Pool(processes=48) as pool:
            args = [(catId, cocoDt, cocoGt, catId, iou_type, areas)
        ...
        for k, catId in enumerate(catIds):
            ...
            ps[5, :, k, :, :] = ps[4, :, k, :, :] > 0
            ps[6, :, k, :, :] = 1.0
            makeplot(recThrs, ps[:, :, k], res_out_dir, nm['name'], iou_type,ax)
            if extraplots:
                makebarplot(recThrs, ps[:, :, k], res_out_dir, nm['name'],
                            iou_type)
        makeplot(recThrs, ps, res_out_dir, 'allclass', iou_type, ax) # 单独一个全部类别的
        if extraplots:
...
"""
/hy-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_solar_cell/tood_swin-s-p4-w12_fpn_ms-2x_coco_solar_cell.bbox.json
/hy-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_solar_cell/coco_error_analysis_results
--ann=/hy-tmp/mmdetection/mmdet/data/solar_cell_dataset_coco/annotations/instances_val2017.json

/hy-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_new_solar_data/tood_swin-s-p4-w12_fpn_ms-2x_coco_solar_cell.bbox.json
/hy-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_new_solar_data/coco_error_analysis_results
--ann=/hy-tmp/mmdetection/mmdet/data/solar_cell_dataset_coco/annotations/instances_val2017.json

/hy-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone.bbox.json
/hy-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone/coco_error_analysis_results
--ann=/hy-tmp/mmdetection/mmdet/data/solar_cell_dataset_coco/annotations/instances_val2017.json

/root/autodl-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone-EL-image-paapn-data3-process-orin-bs4.bbox.json
/root/autodl-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone-EL-image-paapn-data3-process-orin-bs4/coco_error_analysis_results
--ann=/root/autodl-tmp/mmdetection/mmdet/data/solar_cell_EL_image_coco/annotations/instances_val2017.json



/root/autodl-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone-EL-image-apn-data3-process-orin/bboxjson.bbox.json
/root/autodl-tmp/mmdetection/tools/work_dirs/tood_swin-s-p4-w12_fpn_ms-2x_coco_multi_backbone-EL-image-apn-data3-process-orin/coco_error_analysis_results
--ann=/root/autodl-tmp/mmdetection/mmdet/data/solar_cell_EL_image_coco/annotations/instances_val2017.json
