...
def visualize_attention_heatmaps(model, inputs):
    """
    可视化注意力热力图
    :param model: 训练好的PyramidAttentions模型
    :param inputs: 输入特征图的列表 [F1, F2, F3, F4, F5]
    :return: None
    """
    # 确保模型处于评估模式
    model.eval()
    ...
    with torch.no_grad():
        # 获取注意力输出
        attentions = model(inputs)

        # 对每一层的注意力进行可视化
        for i, attention in enumerate(attentions):
            # 计算注意力热力图
            heatmap = torch.mean(attention, dim=1).squeeze().cpu().numpy()
...
class FPN_HCBP_ACRIS_APN_5(BaseModule):
