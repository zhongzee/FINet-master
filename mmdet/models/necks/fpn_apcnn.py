...
# 新增
import random
from .nms import nms_pytorch as pth_nms
...
#
#     def __init__(self, channel_size=256, num_inputs=5):
...
#
    ...
    #     assert len(inputs) == len(self.attentions)
    #     spatial_attentions = []
    #     channel_attentions = []
    #
    #     for i in range(len(inputs)):
    #         spatial = self.attentions[i][0](inputs[i])
    #         channel = self.attentions[i][1](inputs[i])
    #         spatial_attentions.append(spatial)
    #         channel_attentions.append(channel)
    #
    #     combined_attentions = []
    #     for i in range(len(inputs)):
    #         spatial = spatial_attentions[i]
    #         channel = channel_attentions[i]
    #         for j in range(i + 1, len(inputs)-1):
    #             channel = (channel + channel_attentions[j]) / 2
    #         combined = spatial * inputs[i] + channel * inputs[i]
    #         combined_attentions.append(combined)
    #         combined_attentions.append(spatial)
    #
...
class FPN(BaseModule):
    ...
    def __init__(
        ...
        # 初始化 FeatureMapping 层
        # self.feature_mapping_layers = nn.ModuleList([
        #     FeatureMapping(input_dim=2048, output_dim=c * h * w,out_channels=c, height=h, width=w)
        #     for c, h, w in zip(self.in_channels, [92, 46, 23], [92, 46, 23])
        # ])
        # self.feature_mapping_layers = nn.ModuleList([])
        # self.fpn = FPN(in_channels=in_channels, out_channels=out_channels, num_outs=len(self.in_channels)).eval()
    ...
    def forward(self, inputs: Tuple[Tensor]) -> tuple:
        ...
        combined_feats = [feat + apn_feat for feat, apn_feat in zip(outs, apn_out[:num_inputs])]
        # combined_feats.append(inputs[0])
        return [combined_feats, apn_out, inputs]#把backbone输出的特征和fpn结构都传回来，后续有用
        # return tuple(outs)
