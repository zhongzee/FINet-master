...
#
#     def forward(self, inputs):
#         F1, F2, F3, F4, F5 = inputs#这里通道数应该不一样
#
#         A1_spatial = self.A1_1(F1)
#         A1_channel = self.A1_2(F1)
#         A1 = A1_spatial * F1 + A1_channel * F1
#
#         A2_spatial = self.A3_1(F2)
#         A2_channel = self.A3_2(F2)
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        x1 = self.layer1(x)
        x2 = self.layer2(x1)#第2层对x1得到的结果
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)

        # stage I
        f3, f4, f5 = self.fpn([x2, x3, x4])
        f3_att, f4_att, f5_att, a3, a4, a5 = self.apn([f3, f4, f5])#a3, a4, a5只有空间

        # feature concat # 在这里做注意力金字塔？
        f_concate = self.Concate(f3, f4, f5) # 这里将FPN的输出和经过apn的输出组合到一起了
        ...
        # stage II # DropBlock操作？ ROI guided refinement # 只对低层次特征进行处理
