...
# class PSA(nn.Module):
#     def __init__(self, channel=256, reduction=4, S=4):
#         super().__init__()
#         self.S = S
...
#         SE_out = torch.stack(se_out, dim=1).expand_as(SPC_out)
#         softmax_out = self.softmax(SE_out)
#         PSA_out = (SPC_out * softmax_out).view(b, -1, h, w)
#         return PSA_out
#
...
#         self.psa = PSA(channel=out_channels)
#
#     def forward(self, x):
#         return self.psa(x)
...
class AdaptiveAttention(nn.Module):
    def __init__(self, channel_size=256):
        ...
        self.spatial_gate = SpatialGate(channel_size)
        self.channel_gate = ChannelGate(channel_size)
        self.alpha = Parameter(torch.zeros(1))

    def forward(self, x):
        spatial_attention = self.spatial_gate(x)
        channel_attention = self.channel_gate(x)
        attention = self.alpha * spatial_attention + (1 - self.alpha) * channel_attention
        return attention * x, channel_attention


class PyramidAttentions(nn.Module):
    def __init__(self, channel_size=256):
        super(PyramidAttentions, self).__init__()
        self.A1 = AdaptiveAttention(channel_size)
        self.A2 = AdaptiveAttention(channel_size)
        self.A3 = AdaptiveAttention(channel_size)
        self.A4 = AdaptiveAttention(channel_size)
        self.A5 = AdaptiveAttention(channel_size)

    def forward(self, inputs):
        F1, F2, F3, F4, F5 = inputs

        A1, prev_channel_attention = self.A1(F1)
        A2, channel_attention = self.A2(F2)
        A2 = A2 * (prev_channel_attention + channel_attention) / 2
        prev_channel_attention = channel_attention

        A3, channel_attention = self.A3(F3)
        A3 = A3 * (prev_channel_attention + channel_attention) / 2
        prev_channel_attention = channel_attention

        A4, channel_attention = self.A4(F4)
        A4 = A4 * (prev_channel_attention + channel_attention) / 2
        prev_channel_attention = channel_attention

        A5, channel_attention = self.A5(F5)
        A5 = A5 * (prev_channel_attention + channel_attention) / 2
