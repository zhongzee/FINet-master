...
class APINet(nn.Module):
    def __init__(self, config):
        ...
        layers = list(resnet.children())[:-2]#取出resnet的倒数2层之前
    ...
    def forward(self, images, targets=None, flag='train'):
        ...
        conv_out = self.backbone(images)#提取出特征
        pool_out = self.avg(conv_out).squeeze()

        if flag == 'train':
            intra_pairs, inter_pairs, intra_labels, inter_labels = self.get_pairs(pool_out, targets) # 在这里得到self_logits和other_logits
            ...
            return self_logits, other_logits, labels1, labels2 # 因为API-Net backbone
