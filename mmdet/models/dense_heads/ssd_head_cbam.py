...
class SSDHeadCBAM(AnchorHead):
    ...
    def __init__(
        ...
        self.cbam = CBAMBlock(channel=128, reduction=16, kernel_size=49)  # 参数暂用默认的
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List[Tensor], List[Tensor]]:
        ...
        # 仅对最后一层添加cbam注意力机制
        pred_outs = x[:-1]
        last_outs = self.cbam(x[len(x)-1])
        # 返回tuple类型
        x = tuple(list(pred_outs)+[last_outs])
