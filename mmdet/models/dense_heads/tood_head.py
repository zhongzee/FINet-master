...
class TOODHead(ATSSHead):
    ...
    Args:
        ...
        num_dcn (int): Number of deformable convolution in the head.
         ...
         (str): If set to ``anchor_free``, the head will use centers
    ...
    def _init_layers(self) -> None:
        ...
        self.channel_adjust = nn.Conv2d(192, 256, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[List[Tensor]]:
        ...
        cls_scores = []
        bbox_preds = []
        # half_length = len(feats) // 2
        # feats = feats[:half_length]
        # f1_att, f2_att, f3_att, f4_att, f5_att就是原来的feats
        # [apn_out, x2] = feats
        ...
        for idx, (x, scale, stride) in enumerate(
                zip(feats_input, self.scales, self.prior_generator.strides)):
            b, c, h, w = x.shape
    ...
    def loss_by_feat_single(self, anchors: Tensor, cls_score: Tensor,
        ...
        loss_cls = cls_loss_func(#这里应该就是调用的gaussian_focal_loss
    ...
    def loss_by_feat(
        ...
        cls_reg_targets = self.get_targets(#这里用到了，里面的_get_targets_single 调用了样本分配策略
            flatten_cls_scores,
            flatten_bbox_preds,
            anchor_list,
            valid_flag_list,
        ...
        losses_cls, losses_bbox, \
            cls_avg_factors, bbox_avg_factors = multi_apply(
                self.loss_by_feat_single,#调用了loss_by_feat_single
                anchor_list,
                cls_scores,
                bbox_preds,
                labels_list,
    ...
    def get_targets(self,
        ...
        有效标志等信息，以及图像元数据和实例数据等信息。然后它通过调用内部函数来计算每个图像的目标，
        得到每个级别的锚点、标签、标签权重、边界框目标和标准化对齐度量，并返回这些值的列表。
        ...
        if self.epoch < self.initial_epoch:
            (all_anchors, all_labels, all_label_weights, all_bbox_targets,
             ...
             sampling_result) = multi_apply(
                 super()._get_targets_single,#这里用到了样本分配策略
                 anchor_list,
                 valid_flag_list,
            ...
            (all_anchors, all_labels, all_label_weights, all_bbox_targets,
             all_assign_metrics) = multi_apply(
                 self._get_targets_single,#这里用到了
                 cls_scores,
                 bbox_preds,
                 anchor_list,
                 valid_flag_list,
    ...
    def _get_targets_single(self,
        ...
        # 根据锚框和GT实例之间的IoU和位置信息计算得分，使用ATSS方法分配正负样本，并计算对齐度量。（这里应该是TaskAlignedAssigner）
        assign_result = self.alignment_assigner.assign(pred_instances,#预测的锚框
                                                       gt_instances,#GT实例
                                                       gt_instances_ignore,
