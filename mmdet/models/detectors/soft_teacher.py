...
class SoftTeacher(SemiBaseDetector):
    ...
    def get_pseudo_instances(
        ...
        # batch_data_samples 这里长度是4 也就是 4个无标签数据
        ...
        # 通过调用 filter_gt_instances 函数，根据指定的分数阈值 self.semi_train_cfg.pseudo_label_initial_score_thr 过滤伪标签数据中的检测结果。这个阈值用于筛选出可靠的伪标签数据。
        batch_data_samples = filter_gt_instances(
            batch_data_samples,
            score_thr=self.semi_train_cfg.pseudo_label_initial_score_thr)
        # 接下来，调用 self.compute_uncertainty_with_aug 方法，计算了伪标签数据中每个伪标签框的不确定性（uncertainty）。不确定性是一个反映了模型对每个伪标签框预测的置信度的度量。
    ...
    def rcnn_reg_loss_by_pseudo_instances(
        ...
        # 具体来说，这段代码的作用是计算目标检测中的回归损失（bbox regression loss），而 reg_uncs 用于筛选出哪些伪标签数据中的候选框将被用于计算回归损失。
        for data_samples in reg_data_samples:
            if data_samples.gt_instances.bboxes.shape[0] > 0:
                # 通过 self.semi_train_cfg.reg_pseudo_thr（半监督训练的阈值参数）将不确定性小于阈值的伪标签框保留下来。
